...
function manger_ajax_login() {
    $("form#manger_login").submit(
	    function() {
		var I = this;
		if (this.manger_login_username.value == ""
			|| this.manger_login_password.value == "") {
...
function click_delete_user(id, username) {
    ...
    $("#addevent .modal-body label.control-label[for='inputPassword']").html(
	    "新密码");
	    ...
	    .post(
			...
			if (d.code == 0) {
			    showMessage(
				    d.message,
				    function() {
					window.location = "manger_block.php?depart_id=<?php echo $depart_id; ?>";
				    }, 4000);
			} else {
			    showMessage(d.message);
			}

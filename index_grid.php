...
<?php
$sql = "select * from `record_record` where lockauthor = '' ORDER BY  `time` DESC LIMIT ".($nowPage-1)*$pageSize." , $pageSize";
...
while($row=@mysql_fetch_array($result)){
    ...
    if(strcmp($admin,"record_admin") == 0){
        $alter = MAIN_DOMAIN."alter.php?id=$id";
        $delete = MAIN_DOMAIN."alter.php?id=$id";
    }
    ...
    echo "
        ...
        <a href=\"$alter\" class=\"_editBtn\"><span>编辑</span></a>
        </div>
        <div class=\"del-btn\" title=\"删除\">
        <a href=\"$delete\" class=\"_delBtn\"><span>删除</span></a>
        </div>
        </div>
        <div class=\"unit-box\">
        <div class=\"unit-c\">
